  ...
  useEffect(() => {
    if (currentSession && !currentSession.summary && !summaryLoading && isSessionDone) {
      ...
      aiService.generateSessionSummary(currentSession)
        .then((summary) => { setSummary(summary); setAiSummary(summary); })
    ...
    <div className="min-h-screen flex flex-col items-center px-4 py-12 md:py-16">
      ...
      <div className="w-full max-w-lg animate-fade-in">
        ...
        <div className="text-center mb-10 animate-fade-in">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-sage-50 rounded-3xl mb-5 animate-scale-in">
            {isSessionDone ? <Sparkles size={28} className="text-sage-500" /> : <Heart size={28} className="text-warm-400" />}
          </div>
          <h1 className="font-serif text-3xl md:text-4xl text-text-primary mb-3">{isSessionDone ? 'You did it.' : 'Good work today.'}</h1>
          {summaryLoading ? (
            <div className="flex justify-center gap-1.5 py-2">
              {[0, 1, 2].map(i => (<div key={i} className="w-2 h-2 rounded-full bg-sage-300 animate-pulse" style={{ animationDelay: `${i * 200}ms` }} />))}
            </div>
          ) : (
            <p className="text-text-secondary text-lg leading-relaxed animate-fade-in" style={{ animationDelay: '300ms' }}>{aiSummary || currentSession?.summary}</p>
          )}
        </div>

        <div className="space-y-4 mb-8 animate-fade-in" style={{ animationDelay: '200ms' }}>
          ...
          <div className="mb-8 animate-fade-in" style={{ animationDelay: '250ms' }}>
            ...
            <div className="grid grid-cols-2 gap-2">
              {completedSteps.map((step, i) => (
                <div key={step.id} className="bg-sage-50 border border-sage-100 rounded-2xl p-3 text-center animate-fade-in" style={{ animationDelay: `${300 + i * 50}ms` }}>
            ...
            <Card>
              <div className="space-y-2">
                {(() => {
                  ...
                  currentSession!.distractions.forEach(d => { counts[d.label] = (counts[d.label] || 0) + 1; });
              ...
              <div className="space-y-3">
                {recentEasy > 0 && (
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1"><div className="h-2 bg-cream-100 rounded-full overflow-hidden"><div className="h-full bg-sage-300 rounded-full transition-all duration-700" style={{ width: `${(recentEasy / profile.recentFeedback.length) * 100}%` }} /></div></div>
                ...
                {recentOkay > 0 && (
                  <div className="flex items-center gap-3">
                    <span className="text-lg">😐</span>
                    <div className="flex-1"><div className="h-2 bg-cream-100 rounded-full overflow-hidden"><div className="h-full bg-warm-300 rounded-full transition-all duration-700" style={{ width: `${(recentOkay / profile.recentFeedback.length) * 100}%` }} /></div></div>
                ...
                {recentHard > 0 && (
                  <div className="flex items-center gap-3">
                    <span className="text-lg">😣</span>
                    <div className="flex-1"><div className="h-2 bg-cream-100 rounded-full overflow-hidden"><div className="h-full bg-warm-400 rounded-full transition-all duration-700" style={{ width: `${(recentHard / profile.recentFeedback.length) * 100}%` }} /></div></div>
          ...
          <div className="space-y-3">
            <Card padding="sm">
              <div className="flex items-start gap-3">
                <div className="p-2 bg-sage-50 rounded-xl shrink-0"><Clock size={16} className="text-sage-500" /></div>
                <div>
            ...
            <Card padding="sm">
              <div className="flex items-start gap-3">
                <div className="p-2 bg-sage-50 rounded-xl shrink-0"><CheckCircle2 size={16} className="text-sage-500" /></div>
                <div>
        ...
        <div className="flex flex-col gap-3 animate-fade-in" style={{ animationDelay: '600ms' }}>
          <Button onClick={resetToHome} size="lg" className="w-full">Start something new<ArrowRight size={18} /></Button>
        </div>
      </div>
    </div>
  );
